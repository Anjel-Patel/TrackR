...
public class RecyclerItemTouchHelper extends ItemTouchHelper.SimpleCallback {
    ...
    public void onChildDraw(Canvas c, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        ...
        if (dX > 0) {
            ...
            background = new ColorDrawable(ContextCompat.getColor(adapter.getContext(), R.color.primary_blue));
        } else {
            icon = ContextCompat.getDrawable(adapter.getContext(), R.drawable.ic_delete);
            background = new ColorDrawable(ContextCompat.getColor(adapter.getContext(), R.color.error_red));
        }
